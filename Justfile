...
gen-sbom $image="bluefin" $tag="latest" $flavor="main" $syft_cmd="syft":
    ...
    ${PODMAN} container create --replace --name ${image_name} \
          "${image_name}:${tag}"
    ...
    ${syft_cmd} --source-name "${image_name}:${tag}" "${OUT_DIR}" \
          -o syft-json=${SBOM}
    du -sh "${SBOM}"
...
local-iso variant="lts" flavor="base" repo="ghcr":
    build_files/iso/local-iso-build.sh {{ variant }} {{ flavor }} {{ repo }}


# Build a local Fedora-based Bluefin ISO using Titanoboa
[group('ISO')]
local-iso-bluefin flavor="base" repo="ghcr":
    just local-iso silver-ublue {{ flavor }} {{ repo }}
