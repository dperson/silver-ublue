...
build $image="silver-ublue" $tag="latest" $flavor="main" rechunk="0" ghcr="0" pipeline="0" $kernel_pin="":
    ...
    #   {{ just }} verify-container \
    #         "akmods-nvidia:${akmods_flavor}-${fedora_version}-${kernel_release}"
    fi
    ...
    LABELS+=("--label" "org.opencontainers.image.description=Custom silver/ublue build ")
    ...
    # Build Image
    PODMAN_BUILD_ARGS=("${BUILD_ARGS[@]}" "${LABELS[@]}" --target "${target}")
...
fedora_version image="silver-ublue" tag="latest" flavor="main" $kernel_pin="":
    ...
    fedora_version=$(jq -r '.Labels["ostree.linux"]' </tmp/manifest.json |
          grep -oP 'fc\K[0-9]+')
...
retag-nvidia-on-ghcr working_tag="" stream="" dry_run="1":
    ...
    for image in silver-ublue-nvidia-open; do # silver-ublue-nvidia
