...
for i in $(dconf list /org/gnome/Ptyxis/Profiles/); do
  i=${i:0:-1}
  [[ $(dconf read /org/gnome/Ptyxis/profile-uuids) =~ $i ]] ||
        dconf reset -f "/org/gnome/Ptyxis/Profiles/${i}/"
done
...
for i in "${!array[@]}"; do
  ...
  if test "$ublue_os" = "true"; then
    ...
    if ! test "$DEFAULT_VALUE" = "$guid" &&
          test "$name" = "$label" &&
          ! systemctl --user --quiet is-enabled "${name}".target; then
