...
for i in gnome-shell mesa-filesystem mesa-dri-drivers mesa-libEGL mesa-libGL \
  ...
  dnf5 swap --repo="terra, terra-extras, terra-mesa" \
        --setopt=install_weak_deps=False -y "$i" "$i"
  dnf5 versionlock add "$i"
done

# Fix for ID in fwupd
# dnf5 swap --repo=copr:copr.fedorainfracloud.org:ublue-os:staging \
...
dnf5 install --setopt=install_weak_deps=False -y /tmp/dive.rpm
get_git_pkg "fluxcd/flux2" "linux_amd64.tar.gz$" /tmp/flux.tgz flux
tar -C /usr/bin -xf /tmp/flux.tgz flux
url="https://raw.githubusercontent.com/helm/helm/main/scripts/get-helm-3"
ghcurl "$url" -o /tmp/get_helm.sh
env HELM_INSTALL_DIR=/usr/bin bash /tmp/get_helm.sh
get_git_pkg "derailed/k9s" "linux_amd64.rpm$" /tmp/k9s.rpm
dnf5 install --setopt=install_weak_deps=False -y /tmp/k9s.rpm
...
chown root: /usr/bin/{flux,kind,ko,kubectl,kubectx,kubens,mc}
chmod 0755 /usr/bin/{flux,kind,ko,kubectl,kubectx,kubens,mc}
rm /tmp/{dive.rpm,flux.tgz,get_helm.sh,k9s.rpm,ko.tgz}
...
gext -F install 615  # appindicatorsupport@rgcjonas.gmail.com
gext -F install 16 # auto-move-windows@gnome-shell-extensions.gcampax.github.com
gext -F install 6670 # Bluetooth-Battery-Meter@maniacx.github.com
gext -F install 3193 # blur-my-shell@aunetx
gext -F install 517  # caffeine@patapon.info
gext -F install 4839 # clipboard-history@alexsaveau.dev
gext -F install 307  # dash-to-dock@micxgx.gmail.com
gext -F install 3843 # just-perfection-desktop@just-perfection
gext -F install 7048 # rounded-window-corners@fxgn
gext -F install 6139 # tailscale@joaophi.github.com
# gext -F install 5263 # gtk4-ding@smedius.gitlab.com
gext -F install 7065 # tilingshell@ferrarodomenico.com
gext -F install 6784 # wiggle@mechtifs
mv /root/.local/share/gnome-shell/extensions/* /usr/share/gnome-shell/extensions
find /usr/share/gnome-shell/extensions -iname \*.gschema.xml|while read -r i; do
  (cd "$(dirname "$i")" && glib-compile-schemas .)
done
echo "::endgroup::"