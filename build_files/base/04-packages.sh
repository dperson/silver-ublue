...
if [[ ! "${IMAGE_NAME}" =~ nvidia ]]; then
  ...
  # rocm-hip
  rm /usr/libexec/ublue-nvidia-flatpak-runtime-sync \
        /usr/lib/systemd/system/ublue-nvidia-flatpak-runtime-sync.service
fi

# Version-specific Fedora package additions
case "$FEDORA_MAJOR_VERSION" in
  43) FEDORA_PACKAGES+=(gnupg2-scdaemon) ;;
  44) FEDORA_PACKAGES+=(gnupg2-scdaemon) ;;
esac
...
case "$FEDORA_MAJOR_VERSION" in
  ...
  # 44) copr_install_isolated "ublue-os/staging" "fw-ectool" "fw-fanctrl" ;;
esac
...
dnf install --enablerepo='terra' --setopt=install_weak_deps=False -y ghostty \
      ghostty-shell-integration ghostty-zsh-completion
# if [[ ! "${IMAGE_NAME}" =~ nvidia ]]; then
#   dnf swap --repo="terra, terra-extras, terra-mesa" \
#         --setopt=install_weak_deps=False -y mesa-va-drivers mesa-dri-drivers
#   dnf swap --repo="terra, terra-extras, terra-mesa" \
...
file=/usr/lib/systemd/system/flatpak-add-fedora-repos.service
[[ -e $file ]] && rm -f "$file"
