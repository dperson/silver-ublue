...
dnf5 install --setopt=install_weak_deps=False -y "${FEDORA_PACKAGES[@]}"
...
#   43) EXCLUDED_PACKAGES+=(fwupd gnome-software) ;;
# esac
...
if [[ "${FEDORA_MAJOR_VERSION}" -gt 41 ]]; then
  ...
  for i in flatpak flatpak-libs flatpak-session-helper; do
    dnf5 swap --repo=copr:copr.fedorainfracloud.org:ublue-os:flatpak-test \
          --setopt=install_weak_deps=False -y "$i" "$i"
  done
