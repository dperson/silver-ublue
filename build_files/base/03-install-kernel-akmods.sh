...
if [[ "${IMAGE_NAME}" =~ nvidia ]]; then
  ...
  if [[ "${IMAGE_NAME}" =~ open ]]; then
    ...
    skopeo copy --retry-times 3 "${url}-$(rpm -E %fedora)-${KERNEL}" \
          dir:/tmp/akmods-rpms
  # else
  #   url="docker://ghcr.io/ublue-os/akmods-nvidia:${AKMODS_FLAVOR}"
  #   skopeo copy --retry-times 3 "${url}-$(rpm -E %fedora)-${KERNEL}" \
  #         dir:/tmp/akmods-rpms
  fi
