...
copr_install_isolated() {
  ...
  dnf5 copr enable -y "$copr_name"
  dnf5 copr disable -y "$copr_name"
  dnf5 install --enablerepo="$repo_id" --setopt=install_weak_deps=False -y \
        "${packages[@]}"
...
thirdparty_repo_install() {
  ...
  dnf5 install --nogpgcheck --repofrompath "$repo_frompath" -y \
        "$release_package"

  # Install extras package if specified (may not exist in all versions)
  if [[ -n "$extras_package" ]]; then
    dnf5 install --setopt=install_weak_deps=False -y "$extras_package" || :
  fi
