...
echo "NOTE: With secure isolated installation, COPRs shouldn't be enabled!"
...
echo -e "\nChecking Fedora updates-testing (should be disabled unless beta)..."
if [[ -f "$REPOS_DIR/fedora-updates-testing.repo" ]]; then
  if grep -q "^enabled=1" "$REPOS_DIR/fedora-updates-testing.repo" 2>/dev/null
  then
    ...
    if [[ "${UBLUE_IMAGE_TAG:-stable}" == "beta" ]]; then
      ...
      echo "ENABLED: fedora-updates-testing.repo (only enabled for beta)"
