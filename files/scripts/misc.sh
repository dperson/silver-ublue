...
# Install kanata
P='.[0].assets[]|select(.name|test("^kanata$"))|.browser_download_url'
URL="https://api.github.com/repos/jtroo/kanata/releases"
curl -LSs "$(curl -LSs "${URL}" | jq -r "${P}")" -o /usr/bin/kanata
