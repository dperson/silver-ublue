...
#curl -LSs "${URL}/macrecovery" -o /usr/bin/macrecovery
#curl -LSs "${URL}/quickemu" -o /usr/bin/quickemu
#curl -LSs "${URL}/quickget" -o /usr/bin/quickget
#curl -LSs "${URL}/windowskey" -o /usr/bin/windowskey
...
# Fix open files and timeouts on shutdown to be sane
if [[ -e /etc/systemd/user.conf ]]; then
  sed -i 's/#\(DefaultLimitNOFILE\).*/\1=4096:524288/' /etc/systemd/user.conf
  sed -i 's/#\(DefaultTimeoutStopSec\).*/\1=15s/' /etc/systemd/user.conf
elif [[ -e /usr/etc/systemd/user.conf ]]; then
  sed -i 's/#\(DefaultLimitNOFILE\).*/\1=4096:524288/' \
        /usr/etc/systemd/user.conf
  sed -i 's/#\(DefaultTimeoutStopSec\).*/\1=15s/' /usr/etc/systemd/user.conf
fi
if [[ -e /etc/systemd/system.conf ]]; then
  sed -i 's/#\(DefaultTimeoutStopSec\).*/\1=15s/' /etc/systemd/system.conf
elif [[ -e /usr/etc/systemd/system.conf ]]; then
  sed -i 's/#\(DefaultTimeoutStopSec\).*/\1=15s/' /usr/etc/systemd/system.conf
fi
