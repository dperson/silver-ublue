...
echo "Setting up container signing in policy.json & cosign.yaml for $IMAGE_NAME"
...
cp /usr/etc/containers/registries.d/ublue-os.yaml \
      /usr/etc/containers/registries.d/"$IMAGE_NAME".yaml
sed -i "s ghcr.io/ublue-os $IMAGE_REGISTRY g" \
      /usr/etc/containers/registries.d/"$IMAGE_NAME".yaml